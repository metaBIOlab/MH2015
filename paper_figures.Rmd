...
```{r}
## Importa os dados
dados_MH2015 <- read_excel("dados_Bruno_etal_Nature_UniversalScaling.xls")
View(dados_MH2015)
...
Verificando o expoente com o desvio padrao:
```{r}
lm <- lm(log10(TotalArea*sqrt(AvgThickness)) ~ log10(ExposedArea), filter(dados_MH2015, FI > 1))
summary(lm)
```

```{r}
cor <- cor.test(log10(filter(dados_MH2015, FI > 1)$ExposedArea), log10(filter(dados_MH2015, FI > 1)$TotalArea*sqrt(filter(dados_MH2015, FI > 1)$AvgThickness)))
cor
paste("R2 = ", signif(cor$estimate^2, 2))
```
...
ggplot(filter(dados_MH2015), aes(x = ExposedArea, y = AvgThickness, color = clade, fill = clade)) +
  ...
  labs(x = "Exposed cortical surface area (mm²)", y = "AvgThickness (mm)") +
  theme_minimal() +
  stat_smooth(data = subset(dados_MH2015, FI >1), method = "lm", alpha = 0.4) +
  stat_regline_equation(data = subset(dados_MH2015, FI >1))
```
